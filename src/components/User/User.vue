			...
			<div class="relative flex items-end justify-start h-full lg:ml-[1rem]">
				...
				>
					<img
						...
						class="absolute left-0 top-0 object-fit rounded-full"
				...
				>
					<div>
						<h2 class="text-[12px] leading-4 font-semibold">PROFILE</h2>
					</div>
					<h1
					...
					>
						...
						>
							<span v-if="publicPlaylists?.length === 1" class="">
						...
						<div class="flex shrink-0 wrap">
							<router-link
								v-if="currentUserTotalFollowers.length === 1"
								to="/"
								class="hover:underline before:content-['•'] before:mx-1 before:inline-block"
							>
								{{ currentUserTotalFollowers }} Follower
							</router-link>
							<router-link
								v-else-if="currentUserTotalFollowers > 1"
								to="/"
								class="hover:underline before:content-['•'] before:mx-1 before:inline-block"
							>
								{{ currentUserTotalFollowers }} Followers
							</router-link>
						</div>
						<div class="flex shrink-0 wrap">
							<router-link
								to="/"
								class="hover:underline before:content-['•'] before:mx-1 before:inline-block"
								>{{ curUserFollowedArtists.length }} Following
							</router-link>
						</div>
					</div>
				</div>
				...
				<div class="flex justify-start items-center">
					...
					>
						<button
							...
							class="flex items-center justify-center border cursor-default w-[5.5rem] h-fit text-white py-[6px] rounded-[5px] tracking-wider text-[12px]"
						>
							<span v-if="currentUserIsFav" style="font-weight: 700"
								>FOLLOWING</span
							>
							<span v-else style="font-weight: 700"> FOLLOW</span>
						</button>
					</div>
				</div>
		...
		<div v-if="isMyUserPage">
			...
			<section class="p-5 lg:ml-[1rem]">
				<Card :userPage="userPage" :currentData="currentUserTopArtists">
					<template #cardTitle>Top artists this month</template>
					...
					<template #playBtn="{ data }">
						<div
							...
							class="bg-dark1 rounded-full right-0 bottom-0 absolute flex items-center my-1 mx-1 group-hover:block group-hover:opacity-100 transition ease-in duration-200 group-hover:translate-y-[-0.4rem]"
						>
							<button
								@click="
			...
			<section class="p-5 lg:ml-[1rem]">
				<div class="relative">
					<div class="flex justify-between items-stretch h-[3.3rem]">
						<div class="h-fit flex flex-col gap-y-1 flex-end">
							<h2
							...
							<h6 class="text-lightest text-sm">Only visible to you</h6>
						</div>
						<div class="self-end">
							<h6
								style="font-weight: 700"
								class="h-full md:text-[12px] text-[10px] leading-10 text-lightest hover:underline hover:text-white underline-offset-2 cursor-pointer"
							>
								Show all
							</h6>
						</div>
	...
	S46.911,43,42.5,43z M21.5,5.878l29-3.741v3.983l-29,3.741V5.878z"
				...
				<template #secondTitle="{ data }"
					><span v-if="totalFollowers[data.id] !== 0"
						>{{ totalFollowers[data.id] }} Followers</span
					>
				</template>
				<template #playBtn="{ data }">
					<div
		...
		<!-- Followers -->
		<section v-if="false" class="p-5 lg:ml-[1rem]">
			<Card
				:follows="true"
				:artists="true"
				:currentData="curUserFollowedArtists"
			>
				<template #cardTitle>Followers</template>
