			...
			<div class="w-full bg-dark items-stretch grid grid-cols-8 mt-4 gap-6">
				...
				>
					...
					>
						...
						<div class="m-2 self-start w-24 h-24 rounded-full">
							<img
								class="rounded-full w-24 h-24"
								:src="getTopResult?.images[0].url"
								alt="deneme"
							/>
						</div>
						<div class="p-2">
						...
						<div class="p-2">
							<p
								...
								class="w-fit rounded-full bg-dark1 text-white text-xs px-3 py-2"
							>
								{{ getTopResult?.type }}
							</p>
						</div>
...
export default {
	...
	methods: {
		...
		topResult() {
			...
			console.log(this.getSearchResult[this.selectedType].items[0]);
			console.log(this.getSearchResult[this.selectedType].items[0].name);
			let firstBox = { ...this.getSearchResult[this.selectedType].items[0] };
			this.$store.dispatch('searchItem/topResult', firstBox);

			console.log(firstBox);
			console.log(firstBox?.images[0].url);
		},
		topSongs() {
