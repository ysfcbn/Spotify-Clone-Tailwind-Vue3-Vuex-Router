<template>
  <section class="Main-Page p-5 mt-2 lg:ml-[1rem]">
    ...
    <Card
      v-for="(item, i) in getRandomSelectedPlaylists"
      :key="item"
      @click="openPlaylist(msg, $event)"
      :currentData="getRandomSelectedPlaylists[i]?.playlists?.items"
    >
...
export default {
  ...
  methods: {
    ...
    openPlaylist(_, e) {
      this.playlistID = e.target.closest(".card--container").id;
      this.$router.push(`/playlist/${this.playlistID}`);
    },
    openShow(_, e) {
      this.showID = e.target.closest(".card--container").id;
  ...
  computed: {
    ...
    severalPlaylists() {
      return this.$store.getters["playlists/getseveralPlaylists"]?.playlists
        ?.items;
    },
    message() {
      return this.severalPlaylists?.message;
    },
    favShows() {
  ...
  async mounted() {
    ...
    if (this.isAuth) {
      ...
      await this.fetchFavAlbums();
      await this.fetchFavEpisodes();
      await this.fetchFavShows();
      await this.fetchBrowseCategories();
      await this.fetchSeveralPlaylists();
      await this.fetchSeveralPlaylists2();
      if (!this.randomIndexs) {
        this.randomNumbers();
