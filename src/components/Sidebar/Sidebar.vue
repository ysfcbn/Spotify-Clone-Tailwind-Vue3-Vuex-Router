<template>
	<div
		...
		class="sidebar min-w-[17rem] bg-black text-[0.8125rem] mb:text-xs md:text-sm"
	>
		<div class="p-5">
		...
		<div class="">
			...
			<router-link
				...
				:to="openCollection"
				class="hover:opacity-100 text-opacwhite3 opacity-70 transition duration-300 group pl-4 cursor-pointer flex items-center font-semibold flex-row mx-2"
				exact
			>
				<span class="flex align-center justify-center rounded-xs">
					<svg role="img" height="24" width="24" viewBox="0 0 24 24">
		...
		<router-link
			...
			:to="openFavSongs"
			class="hover:opacity-100 text-opacwhite3 opacity-70 transition duration-300 pt-4 px-4 w-full flex items-center font-semibold mx-2 relative items-center h-fit"
			exact
		>
			<span
		...
		<router-link
			v-if="isAuth"
			:to="{ name: 'episodes' }"
			class="hover:opacity-100 text-opacwhite3 opacity-70 transition duration-300 relative w-full mt-4 px-4 flex items-center justify-start font-semibold mx-2"
		>
			<span class="w-6 h-6 flex items-center justify-center bg-green4">
				<svg
					role="img"
					height="13"
					width="13"
		...
		>
			<div class="mx-6 flex flex-col h-fit my-2">
				<router-link
					v-for="playlist in getUserFavPlaylists"
					:id="playlist.id"
					:key="playlist.id"
					:to="{ name: 'playlist', params: { id: `${playlist.id}` } }"
					class="text-white w-[14rem] h-fit opacity-50 text-[14px] hover:text-white hover:opacity-100 cursor-default py-[2px] leading-7 truncate"
				>
					{{ playlist.name }}
				</router-link>
			</div>
		...
		<div
			v-if="isAuth"
			id="download--app"
			:class="`absolute bottom-0 bg-black w-full h-[3rem] ${
				!trackImgDisplay ? 'h-[19.5rem] ease-in duration-200' : ''
			}`"
		>
			<button
