	...
	>
		...
		>
			<div class="flex relative z-0 text-white">
				<router-link
					v-if="!getCurrentlyPlayingContextType && !isArtistContext"
					:key="'track'"
					:to="{ name: 'track', params: { id: `${contextTypeID}` } }"
					class="md:text-[17px] lg:text-[18px] shrink-0 font-semibold cursor-pointer hover:underline"
				>
					{{ contextName }}
				</router-link>
				<router-link
					v-else-if="!getCurrentlyPlayingContextType && isArtistContext"
					:key="artistID"
	...
	computed: {
		...
		contextTypeID() {
			return this.getCurrentlyPlayingContextType === 'album'
				...
				: !this.getCurrentlyPlayingContextType && !this.isArtistContext
				? this.currentTrackID
		...
		contextName() {
			return !this.getCurrentlyPlayingContextType && !this.isArtistContext
				? this.getQueueName
				: !this.getCurrentlyPlayingContextType
				? this.artistName
