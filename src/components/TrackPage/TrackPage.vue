<template>
	<section v-if="currentTrack && isAuth" class="box-border">
		...
		>
			<div class="relative flex items-end justify-start h-full lg:ml-[1rem]">
				...
				<div class="flex flex-col text-white px-4">
					<h2 style="font-weight: 700" class="text-xs">SONG</h2>
					<span>
						<h1
		...
		>
			<div class="w-full ml-[1.2rem] flex items-center lg:ml-[2rem]">
				<button
					...
					"
					class="rounded-full bg-green3 p-[12px] cursor-default hover:scale-105"
				>
					<span>
						<svg role="img" height="26" width="26" viewBox="0 0 24 24" class="">
							<path
				...
				<div>
					<button
						@click="unFollowTrack"
						id="heartBtn"
						class="p-[0.9rem] w-[4rem] ml-3 text-lightest cursor-default hover:text-white"
						:class="{
		...
		<section class="p-5 lg:ml-[1rem]">
			...
			<div class="mt-8">
				<p
					class="text-[11px] text-lightest"
					v-for="copy in copyrights"
					:key="copy.type"
				>
					{{ copy.text }}
				</p>
...
export default {
	...
	methods: {
		...
		async playCurrentTrack(uri) {
			...
			if (this.isPlayingCurrentTrack) {
				...
				if (this.getCurrentlyPlayingTrack?.item.id !== this.currentTrackID)
					await this.$store.dispatch('controller/playSelectedTrack', uri);
				await this.$store.dispatch('controller/playCurrentTrack');
			}
		},
	...
	unmounted() {
		...
		this.$store.dispatch('albums/clearTracksID');
		this.$store.dispatch('albums/clearTracksID2');
		this.$store.dispatch('albums/clearCurrentTrack');
