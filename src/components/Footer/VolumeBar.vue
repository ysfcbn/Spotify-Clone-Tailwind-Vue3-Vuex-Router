	...
	>
		<div>
			...
			>
				...
				>
					<path
						:fill="getCurrentTrackInfo ? '#1FDF64' : 'currentColor'"
		...
		<div class="mx-3">
			...
			>
				...
				>
					<path
						:fill="isQueuePage ? '#1FDF64' : 'currentColor'"
		...
		<div class="ml-2">
			<button class="cursor-default">
				...
				>
					...
					></path>
					<path
						:fill="`${highVol ? 'currentColor' : 'none'}`"
						d="M9.741.85a.75.75 0 01.375.65v13a.75.75 0 01-1.125.65l-6.925-4a3.642 3.642 0 01-1.33-4.967 3.639 3.639 0 011.33-1.332l6.925-4a.75.75 0 01.75 0zm-6.924 5.3a2.139 2.139 0 000 3.7l5.8 3.35V2.8l-5.8 3.35zm8.683 4.29V5.56a2.75 2.75 0 010 4.88z"
					></path>
					<path
						:fill="`${highVol ? 'currentColor' : 'none'}`"
		...
		>
			<div class="group flex relative w-full h-1 bg-lightest/40 rounded-full">
				...
				>
					<span
						class="block invisible group-hover:visible bg-white w-[11px] h-[11px] absolute rounded-full top-[-3px] right-[-8px]"
					></span>
				</span>
			</div>
		</div>

		<div class="w-[3rem] mx-1 mb-1">
			<button class="cursor-default">
				<svg
					role="img"
					height="16"
...
export default {
	...
	methods: {
		toggleCurTrackInfo() {
			console.log(this.getCurrentTrackInfo);
			this.getCurrentTrackInfo
				? this.$store.commit('controller/currentTrackInfo', false)
				: this.$store.commit('controller/currentTrackInfo', true);
		},
		toggleQueue() {
			if (this.isQueuePage) {
				this.$router.back();
		...
		volumeLevel() {
			if (this.volumePercent > 0 && this.volumePercent <= 32) {
				this.lowVol = true;
				this.medVol = false;
				this.highVol = false;
				this.mute = false;
			}
			if (this.volumePercent >= 33 && this.volumePercent <= 65) {
				this.medVol = true;
				this.lowVol = false;
				this.highVol = false;
				this.mute = false;
			}
			if (this.volumePercent >= 66 && this.volumePercent <= 100) {
				this.highVol = true;
				this.medVol = false;
				this.lowVol = false;
				this.mute = false;
		...
		muteVolume() {
			if (this.mute) {
				...
				this.$store.dispatch('controller/putVolumePercent', this.percentVol);
			} else {
				this.$store.dispatch('controller/putVolumePercent', 0);
				this.percentVol = 0;
				this.mute = true;
				this.highVol = false;
				this.medVol = false;
		...
		setVolumePercent(_, e) {
			const volPercent = (e.offsetX * 100) / this.volumeBarEl.offsetWidth;
			this.$store.dispatch(
				'controller/putVolumePercent',
	...
	watch: {
		...
		getCurrentTrackInfo(newVal, oldVal) {
			newVal
				? (this.infoVisiblity = 'visible')
				: (this.infoVisiblity = 'hidden');
		},
	},
	mounted() {
		this.volumeLevel();
		this.percentVol = this.volumePercent;
		this.volumeBarEl = [document.querySelector('#volume--bar')][0];
		console.log(typeof this.volumeBarEl);
		console.log(this.volumeBarEl);
	},
};
</script>
