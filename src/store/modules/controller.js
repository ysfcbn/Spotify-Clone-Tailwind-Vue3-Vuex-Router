...
const controllerModule = {
	...
	mutations: {
		...
		isTrackContext(state, val) {
			state.isTrackContext = val;
		},
	...
	actions: {
		...
		async fetchArtist({ getters, dispatch }, href) {
			await axios
				.get(href, {
					headers: {
						Accept: 'application/json',
		...
		async checkUserFavArtist({ getters, dispatch }, artistID) {
			await axios
				.get(
				...
				.then(({ data }) => {
					dispatch('artists/currentArtistIsFav', ...data, { root: true });
				})
				.catch(err => console.log(err));
		},
					...
					if (data) {
						...
						dispatch('fetchPlaybackState')
							.then(() => {
								...
								dispatch('fetchArtist', data.item.artists[0].href);
		...
		async playSelectedTrack({ getters, dispatch, commit, state }, uri) {
			await axios
				.put(
				...
				.then(data => {
					if (data.status === 204) {
						...
						commit('isTrackContext', true);
						commit('queueName', uri.name);
						state.queueTrackList.length
							? commit('allQueueList', state.queueTrackList)
							: commit('allQueueList', []);
						commit('lastProgressMS', getters.getCurrentProgress);
						commit('lastReverseProgressMS', getters.getLastReverseProgressMS);
						dispatch('clearIntervalFunc');
		...
		async playArtitsTopTracks({ getters, commit, dispatch }, uris) {
			await axios
				.put(
					...
					{
						uris: [
							uris.uri[uris.index ? uris.index : 0],
							...uris.uri.slice(uris.index + 1),
						],
						position_ms:
							uris.type === 'artist' &&
	...
	getters: {
		...
		isTrackContext(state) {
			return state.isTrackContext;
		},
