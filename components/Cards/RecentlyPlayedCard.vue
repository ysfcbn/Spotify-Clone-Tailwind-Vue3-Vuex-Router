				...
				<div
					...
					:class="{
						'opacity-100 translate-y-[-0.4rem]':
							contextUri === getCurrentlyPlayingTrack?.context?.uri &&
...
export default {
	...
	methods: {
		...
		async fetchArtist(href = this.item.context.href) {
			await axios
				.get(href, {
					headers: {
						Accept: 'application/json',
	...
	computed: {
		...
		findArtistsID() {
			return (
				this.getCurrentlyPlayingTrack?.item?.artists[0].id === this.artistID
			);
		},
