<template>
  <ul
    :class="{
      'top-[2.2rem] left-[7rem]': episodes || podcastShowPage,
  ...
  methods: {
    ...
    close(e) {
      if (!this.$el.contains(e.target)) {
        if (this.episodePage || this.podcastShowPage || this.episodes)
          this.$emit("toggleEpisodeOptions", false);
        console.log(
          "clickledi",
          this.episodePage || this.podcastShowPage || this.episodes
        );
      }
    },
  },
